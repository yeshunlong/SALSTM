...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(1, image.shape[0] - 1):
            slice = image[ind, :, :]
            prev_slice = image[ind - 1, :, :]
            next_slice = image[ind + 1, :, :]

            x, y = slice.shape[0], slice.shape[1]
            if x != patch_size[0] or y != patch_size[1]:
                slice = zoom(slice, (patch_size[0] / x, patch_size[1] / y), order=3)  # previous using 0
                prev_slice = zoom(prev_slice, (patch_size[0] / x, patch_size[1] / y), order=3)
                next_slice = zoom(next_slice, (patch_size[0] / x, patch_size[1] / y), order=3)
            input = torch.from_numpy(slice).unsqueeze(0).unsqueeze(0).float().cuda()
            prev_input = torch.from_numpy(prev_slice).unsqueeze(0).unsqueeze(0).float().cuda()
            next_input = torch.from_numpy(next_slice).unsqueeze(0).unsqueeze(0).float().cuda()
            net.eval()
            with torch.no_grad():
                outputs = net(prev_input, input, next_input)
